...
def save_datasets(dataset_path, dataset, n_components, n_elements):
    ...
    if Path(f"{dataset_path}/X_1.npy").exists() and not REFRESH_DATASET:
        print(f"{dataset_path} is non-empty, skipping generation of this dataset")
        return
    X_1, X_23, y_1, y_23 = process_regression_dataset(dataset, n_components=n_components, n_elements=n_elements, test_size=0.66)
    X_2, X_3, y_2, y_3 = process_regression_dataset({'X': X_23, 'y': y_23}, n_components=n_components, test_size=0.50)
    np.save(f"{dataset_path}/X_1.npy", X_1)
    np.save(f"{dataset_path}/X_2.npy", X_2)
    np.save(f"{dataset_path}/X_3.npy", X_3)
    np.save(f"{dataset_path}/y_1.npy", y_1)
    np.save(f"{dataset_path}/y_2.npy", y_2)
    np.save(f"{dataset_path}/y_3.npy", y_3)
...
save_datasets(f"{DATASET_PATH}/fish_market", dataset, 6, 150)
...
save_datasets(f"{DATASET_PATH}/life_expectancy", dataset, 10, 180)
...
save_datasets(f"{DATASET_PATH}/medical_bill", dataset, 6, 180)
...
save_datasets(f"{DATASET_PATH}/ols_cancer", dataset, 10, 180)
...
save_datasets(f"{DATASET_PATH}/real_estate", dataset, 7, 180)
...
def load_dataset_from_fold(dataset_path, fold):
    Xs = [np.load(f"{dataset_path}/X_1.npy"),
          np.load(f"{dataset_path}/X_2.npy"),
          np.load(f"{dataset_path}/X_3.npy")]
    Ys = [np.load(f"{dataset_path}/y_1.npy"),
          np.load(f"{dataset_path}/y_2.npy"),
          np.load(f"{dataset_path}/y_3.npy")]
    tr, v, te = int(fold[2])-1, int(fold[5])-1, int(fold[9])-1
    return Xs[tr], Xs[v], Xs[te], Ys[tr], Ys[v], Ys[te]


def run_simulations(n_layers, epochs, lr):
    ...
    for dataset in DATASETS:
        ...
        X_train, X_validation, X_test, y_train, y_validation, y_test = \
            load_dataset_from_fold(f"{DATASET_PATH}/{dataset}", "TR1-V2-TE3")  # just to load X_train.shape[1]
        ...
        for fold in FOLDS:
            ...
            X_train, X_validation, X_test, y_train, y_validation, y_test = \
                load_dataset_from_fold(f"{DATASET_PATH}/{dataset}", fold)

            print(f"========================= {dataset} {fold} ========================= ")
            save_path = f"{INTERMEDIATE_PATH}/{dataset}/{fold}"
            timing = {}
            ...
            seed = get_next_seed()
            reset_seed(seed)
            run_random_kernel(save_path + "/random_kernel", X_train, X_validation, X_test, y_train, y_validation, y_test, n_layers, seed)
            ...
            seed = get_next_seed()
            reset_seed(seed)
            run_combinatorial_sa_kernel(save_path + "/combinatorial_sa_kernel", X_train, X_validation, X_test, y_train,
                                        y_validation, y_test, n_layers, initial_solution, epochs)
            ...
            seed = get_next_seed()
            reset_seed(seed)
            run_combinatorial_greedy_kernel(save_path + "/combinatorial_greedy_kernel", X_train, X_validation, X_test,
                                        y_train, y_validation, y_test, n_layers, initial_solution)
            timing['combinatorial_greedy_kernel_end'] = datetime.datetime.now().strftime("%Y%m%d%H%M%S")

            json.dump(timing, open(f"{save_path}/timing.json", "w"))


# run_simulations(3, 1000, 0.01)
